...
class Stack {
 public:
  ...
  Stack(Stack &&q) noexcept { container = std::move(q.container); }
  ...
  Stack<T, Container> &operator=(Stack &&q) noexcept {
    //  if (this != &q) { //  по сути эта проверка бессмысленна, перемещение
    //  самого себя приведет к уб и мы не должны от этого защищать
  ...
  */
  ...
  /*
  Exchanges the contents of the container with those of others. Does not invoke
