#ifndef A2_SIMPLENAVIGATOR_CONTAINERS_S21_LIST_H
#define A2_SIMPLENAVIGATOR_CONTAINERS_S21_LIST_H
...
class List {
 ...
 private:
  ...
  class ListIterator {
   public:
    explicit ListIterator(Node *node, Node *carbineNode)
        : currentNode(node), carbineNode(carbineNode) {}
  ...
  class ListCostIterator {
   public:
    explicit ListCostIterator(Node *node, Node *carbineNode)
        : currentNode(node), carbineNode(carbineNode) {}
 ...
 public:
   ...
   */
  reference front() { return carbineNode->next->value; }

  const_reference front() const { return carbineNode->next->value; }

  /*
  Returns a reference to the last element in the container.
  Calling back on an empty container causes undefined behavior.
   */
  reference back() { return carbineNode->prev->value; }

  const_reference back() const { return carbineNode->prev->value; }
  ...
  */
  size_type size() const noexcept { return lSize; }

  /*
  Returns an iterator to the first element of the vector.
  If the vector is empty, the returned iterator will be equal to end()
   */
  iterator begin() noexcept { return iterator(carbineNode->next, carbineNode); }
  ...
  undefined behavior.
   */
  iterator end() noexcept { return iterator(carbineNode, carbineNode); }
  ...
  */
  bool empty() const noexcept { return carbineNode->next == carbineNode; }
  ...
  void push_back(const_reference value) {
    ...
    newNode->prev = carbineNode->prev; //Мы указываем предыдущий узел нового
    узла как узел, который был последним узлом в списке до добавления нового
    элемента. Таким образом, у нового узла будет тот же предыдущий узел, что и у
    последнего элемента списка. newNode->next = carbineNode; // Мы указываем
    следующий узел нового узла как "круговой" узел (фейковый узел), чтобы
    завершить кольцевую структуру списка. carbineNode->prev->next = newNode; //
    Мы обновляем следующий узел у предыдущего узла (последнего узла в списке до
    добавления нового элемента) так, чтобы он указывал на новый узел. Таким
    образом, "старый" последний узел будет указывать на новый узел как
    следующий. carbineNode->prev = newNode; // Мы обновляем предыдущий узел у
    "кругового" узла так, чтобы он указывал на новый узел. Таким образом,
    "круговой" узел будет указывать на новый узел как предыдущий.
    ++lSize;
     */
  }
  ...
  operation is stable: for equivalent elements in the two lists, the elements
  from *this always precede the elements from other, and the order of equivalent
  elements of *this and other does not change.

  No iterators or references to become invalidated. The pointers and references
  to the elements moved from *this, as well as the iterators referring to these
  elements, will refer to the same elements of *this, instead of others.
   */
  ...
  are re-pointed. No iterators or references to become invalidated, the
  iterators to moved elements remain valid, but now refer into *this, not into
  others.

  Transfers all elements from other into *this. The elements are inserted before
...
#endif  // A2_SIMPLENAVIGATOR_CONTAINERS_S21_LIST_H
